...
def test_adjust_bet_buttons(roulette_page: RoulettePage, adjust_bet_button_label, initial_value):
    ...
    expected_value = roulette_page.calculate_bet(float(initial_value), adjust_bet_button_label)
    logger.info(f'Checking that initial value {initial_value} adjusted by {adjust_bet_button_label}; Expected value: {expected_value}')

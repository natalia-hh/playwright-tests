...
class RoulettePage(BasePage):
    ...
    def goto_page(self):
        ...
        self.wait_for_element(
            RouletteLocators.BET_INPUT_FIELD, element_name="Bet Input field"
        )
    ...
    def accept_cookies(self):
        try:
            ...
            if cookie_button.is_visible(timeout=2000):
                self.click(
                    RouletteLocators.COOKIE_BANNER_BUTTON,
                    element_name="Accept Cookie button",
                )
                self.wait_for_element(
                    RouletteLocators.COOKIE_BANNER,
                    state="hidden",
                    element_name="Accept Cookie banner",
                )
            else:
    ...
    def find_adjust_bet_button(self, bet_button: str):
        return self.page.locator(
            f'[data-testid^="roulette-bet-input"] button:not(.button-pill) >> text="{bet_button}"'
        )
    ...
    def click_adjust_bet_button(self, adjust_bet_button_label: str):
        ...
        self.click(
            RouletteLocators.ADJUST_BET_LOCATORS[adjust_bet_button_label],
            element_name=f"Adjust Bet Button {adjust_bet_button_label}",
        )
    ...
    def click_place_bet_button(self, place_bet_button: str):
        ...
        self.click(
            RouletteLocators.PLACE_BET_LOCATORS[place_bet_button],
            element_name=f"Place Bet Button {place_bet_button}",
        )
